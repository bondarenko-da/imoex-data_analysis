...
TOP_STOCKS = [("SBER", "Сбербанк ао"), ("GAZP", "Газпром"), ("LKOH", "Лукойл"), ("ROSN", "Роснефть"), ("GMKN", "Норникель"), ("NVTK", "Новатэк"), ("TATN", "Татнефть ао"), ("MGNT", "Магнит"), ("MOEX", "Московская биржа"), ("YDEX", "Яндекс")]
PERPETUAL_OVERRIDES = {"IMOEXF": "Вечный фьючерс на индекс Мосбиржи", "SBERF": "Вечный фьючерс на Сбербанк", "GAZPF": "Вечный фьючерс на Газпром", "GLDRUBF": "Вечный фьючерс на золото", "SLVRUBF": "Вечный фьючерс на серебро", "USDRUBF": "Вечный фьючерс USD/RUB", "EURRUBF": "Вечный фьючерс EUR/RUB", "CNYRUBF": "Вечный фьючерс CNY/RUB", "RGBIF": "Вечный фьючерс RGBI"}
...
async def ensure_schema() -> None:
    ...
    await conn.execute("""create table if not exists instruments (instrument_id text primary key, secid text, shortname text, display_name text, assetcode text, engine text, market text, boardid text, start_date text, end_date text, expiration_date text, perpetual integer default 0, category text default 'other', updated_at text default (datetime('now')))""")
    await conn.execute("""create table if not exists candles_10m (instrument_id text not null, begin_ts text not null, end_ts text not null, open real, close real, high real, low real, volume real, value integer, primary key (instrument_id, begin_ts))""")
    await conn.commit()
    _SCHEMAS_READY = True
...
async def resolve_instrument(secid: str) -> InstrumentRecord:
    ...
    board = boards[0] if boards else {"engine": "stock", "market": "shares", "boardid": "TQBR", "history_from": None}
    engine = str(board.get("engine", "stock"))
    market = str(board.get("market", "shares"))
    boardid = str(board.get("boardid", "TQBR"))
    shortname = str(descriptions[0].get("SHORTNAME", "") if descriptions else secid) if descriptions else secid
    contract_name = descriptions[0].get("CONTRACTNAME") or descriptions[0].get("NAME") if descriptions else None
    perpetual = str(descriptions[0].get("PERPETUAL_FUTURES", "0") if descriptions else "0") == "1"
    category = "futures" if engine == "futures" else "shares" if market == "shares" else "other"
    return InstrumentRecord(instrument_id=instrument_id(engine, market, boardid, secid), secid=secid, engine=engine, market=market, boardid=boardid, shortname=shortname, display_name=build_display_name(secid, shortname, contract_name, perpetual), assetcode=descriptions[0].get("ASSETCODE") if descriptions else None, start_date=parse_date(board.get("history_from")), end_date=parse_date(board.get("history_till")), expiration_date=parse_date(descriptions[0].get("LSTDELDATE")) if descriptions else None, perpetual=perpetual, category=category)

async def upsert_instrument(record: InstrumentRecord) -> None:
    await ensure_schema()
    conn = await get_sqlite()
    await conn.execute("""insert or replace into instruments (instrument_id, secid, shortname, display_name, assetcode, engine, market, boardid, start_date, end_date, expiration_date, perpetual, category, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))""", (record.instrument_id, record.secid, record.shortname, record.display_name, record.assetcode, record.engine, record.market, record.boardid, str(record.start_date) if record.start_date else None, str(record.end_date) if record.end_date else None, str(record.expiration_date) if record.expiration_date else None, 1 if record.perpetual else 0, record.category))
    await conn.commit()
...
async def sync_candles(record: InstrumentRecord, start_date: date, end_date: date) -> None:
    ...
    offset = 0
    while True:
        payload = await moex_get_json(f"/engines/{record.engine}/markets/{record.market}/boards/{record.boardid}/securities/{record.secid}/candles.json", params={"from": start_date.isoformat(), "till": end_date.isoformat(), "interval": 10, "start": offset})
        candle_rows = rows_to_dicts(payload, "candles")
        if not candle_rows:
            break
        for row in candle_rows:
            rows.append((record.instrument_id, parse_datetime(row["begin"]), parse_datetime(row["end"]), row.get("open"), row.get("close"), row.get("high"), row.get("low"), row.get("volume"), row.get("value")))
...
async def fetch_series(record: InstrumentRecord, start_date: date, end_date: date) -> list[dict[str, Any]]:
    await ensure_schema()
    conn = await get_sqlite()
    cursor = await conn.execute("""select begin_ts, close, open, high, low, volume from candles_10m where instrument_id = ? and begin_ts >= ? and begin_ts < ? order by begin_ts asc""", (record.instrument_id, f"{start_date}T00:00:00", f"{end_date}T23:59:59"))
...
def align_spread(series1: list[dict[str, Any]], series2: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row1 in series1:
        ...
        if not row2:
            continue
        close1, close2 = row1.get("close"), row2.get("close")
        ...
        aligned.append({"ts": row1["ts"], "close1": close1, "close2": close2, "spread": spread, "spread_pct": spread_pct})
    return aligned
...
def monthly_statistics(spread_series: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for month_key in sorted(groups):
        ...
        stats.append({"month": month_key, "month_date": f"{month_key}-01", "observations": len(values), "min_spread": min(values), "median_spread": median(values), "max_spread": max(values), "std_spread": stddev(values)})
    return stats
...
def build_seasonality(monthly_stats_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    matrix = [[by_year[year].get(month) for month in month_order] for year in years]
    return {"months": month_order, "years": years, "values": matrix}

async def build_curated_instruments() -> list[dict[str, str]]:
    futures_payload = await moex_get_json("/engines/futures/markets/forts/securities.json", params={"iss.only": "securities", "securities.columns": "SECID,SHORTNAME,ASSETCODE"})
